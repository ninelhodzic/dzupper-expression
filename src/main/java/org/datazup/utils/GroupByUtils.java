        ...
        Map<Object, List<Object>> result = U.groupBy(list, o -> {
            ...
            if (null != m) {
                if (propertyName.startsWith("#") && propertyName.endsWith("#")){
                    try {
                        return mapperEvaluator.evaluate(propertyName.substring(1, propertyName.length()-1), new PathExtractor(m, mapListResolver));
            ...
            if (null != m) {
                if(null!=properties) {
                    U.each(properties, prop -> {
                        ...
                        /*if (origPropExpression.startsWith("#") && origPropExpression.endsWith("#")){
                            try {
                              origPropVal = mapperEvaluator.evaluate(origPropExpression.substring(1, origPropExpression.length()-1), new PathExtractor(prop, mapListResolver));
                            } catch (EvaluatorException e) {
                                throw new RuntimeException(e);
                            }
                        }else{
                        ...
                        if (StringUtils.containsAny(origPropExpression," ","(",")","$",".")){
                            try {
                                origPropVal = mapperEvaluator.evaluate(origPropExpression, new PathExtractor(m, mapListResolver));
                            } catch (EvaluatorException e) {
                                throw new RuntimeException(e);
                            }
                        }else{
